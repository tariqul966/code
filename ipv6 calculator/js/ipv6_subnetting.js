...
function calculate() {
    ...
    c = c[1];
    if (!checkipv6(a)) {
        document.getElementById("address").innerHTML = "<span style='background-color:yellow;'>This does not look like a valid IPv6 address</span>";
...
function subnet_subnet() {
    var b = document.getElementById("subnets").value;
    ...
    d = d[1];
    if (!checkipv6(l)) {
        document.getElementById("facts").innerHTML = "<span style='background-color:yellow;'>This looks like an invalid IPv6 address</span>";
        return;
    }
    if (splitnewslash(d, b) == false) {
        document.getElementById("facts").innerHTML = "<span style='background-color:yellow;'>Looks like the given IPv6 network cannot be split into this many new subnets</span>";
        return;
    }
...
function subnet_slashes() {
    ...
    var m = "<b>Subnetting " + formatipv6preferred(k) + "/" + d + " into /" + l + "s gives " + b + " subnets, all of which have " + Math.pow(2, 64 - l) + " /64s.</b><p>";
    for (var e = 0; e < b; e++) {
